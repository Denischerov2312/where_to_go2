            ...
            if file_info['name'].endswith('.json'):
                try:
                    ...
                    if 'error' in file_data:
                        raise requests.exceptions.HTTPError(file_data['error'])
                    ...
                    self.stdout.write(self.style.ERROR(f'Ошибка при обработке файла: {file_info["name"]}: {e}'))
                    ...
                    with open(file_path, 'r', encoding='utf-8') as file:
                        ...
                        for number, img_url in enumerate(place_details['imgs'], 1):
                            try:
                                ...
                                self.stdout.write(f'Ошибка загрузки картинки {img_url}: {e}')
                except Exception as e:
                    self.stdout.write(self.style.ERROR(f'Ошибка при обработке {basename(file_path.name)} : {e}'))
